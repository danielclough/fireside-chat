...
impl TextGeneration {
    ...
    pub fn run(&mut self, prompt: &str, sample_len: usize,cache: Option<Cache>,) -> Result<String> {
        ...
        let eos_token = match self.model {
            ...
            _ => {
                self.tokenizer.get_token("</s>").unwrap_or(0)
            }
        };
        ...
        for index in 0..sample_len {
            ...
            let logits = match &mut self.model {
                Model::Mistral(m) => m.forward(&input, context_index).map_err(|x| println!("Error: {x}")),
                Model::QMistral(m) => m.forward(&input, context_index).map_err(|x| println!("Error: {x}")),
                ...
                Model::Llama(m) => {
                    ...
                    m.forward(&input, context_index, cache.unwrap().to_owned().borrow_mut()).map_err(|x| println!("Error: {x}"))
                },
                Model::Llama2(m) => {
                ...
                Model::NoModel(_) => Tensor::randn(0f32, 1., (3, 4), &self.device).map_err(|x| println!("Error: {x}")),
            };
            
            let logits = match &mut self.model {
                Model::Mistral(_) => logits.unwrap().squeeze(0)?.squeeze(0)?.to_dtype(candle_core::DType::F32)?,
